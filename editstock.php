...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // Query untuk mengupdate data penjualan berdasarkan id_pembelian
    $sql = "UPDATE data_beli SET 
...
if (isset($_GET['id_pembelian'])) {
    ...
    $sql = "SELECT * FROM data_beli WHERE id_pembelian = :id_pembelian";
    $stmt = $conn->prepare($sql);
    $stmt->bindParam(':id_pembelian', $id_pembelian, PDO::PARAM_INT);
    $stmt->execute();
        ...
        <?php include 'navbar.php'; 
            ...
            <div id="layoutSidenav_content">
                ...
                <div class="container-fluid">
                        <h1 class="mt-4" style="padding-top: 30px;">Update data</h1>
                        ...
                        <div class="card mb-4">
                            <div class="card-body">
                                Masukkan data stock yang ingin anda edit
                            </div>
                        </div>
                        ...
                        <div class="col-md-6 offset-md-3">
                            <div class="card">
                                <div class="card-header">
                                    Edit Data Pembelian stock
                                </div>
                                <div class="card-body">
                                    <form action="editstock.php" method="POST">
                                        <div class="form-group">
                                        <input type="hidden" name='id_pembelian' id='id_pembelian' value=<?=$row['id_pembelian']?>>
                                        <input type="hidden" name='id_user' id='id_user' value=<?=$row['id_user']?>>
                                            <label for="tgl_pembelian">Tanggal Pembelian</label>
                                            <input type="date" class="form-control" id="tgl_pembelian" name="tgl_pembelian" value="<?=$row['tgl_pembelian']?>"required>
                                        </div>
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" id="id_material" name="id_material" required>
                                                ...
                                                <?php
                                                    ...
                                                    foreach ($material as $material) {
                                                        if ($row['id_material'] == $material['id_material']) {
                                                            echo "<option value='{$material['id_material']}' selected>{$material['nama_material']} ({$material['satuan_material']})</option>";
                                                        }else {
                                        ...
                                        <div class="form-group">
                                            ...
                                            <input type="number" class="form-control" id="jumlah_pembelian" name="jumlah_pembelian" min="1" step="1" value="<?=$row['jumlah_pembelian']?>"required>
                                        </div>
                                        <div class="form-group">
                                            <label for="harga_beli">Harga Bahan Baku(satuan)</label>
                                            <input type="number" class="form-control" id="harga_beli" name="harga_beli" min="1" step="0.01" value="<?=$row['harga_beli']?>" required>
                                        
                                            <input type="hidden" name='total_biaya' id='total_biaya' value=<?=$row['harga_beli']*$row['jumlah_pembelian']?>>
                                        </div>
